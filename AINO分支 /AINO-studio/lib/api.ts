...
async function apiRequest<T>(
  ...
  try {
    ...
    console.log(`📋 Request Config:`, {
      ...
      body: isFormData ? '[form-data]' : (options.body ? JSON.parse(options.body as string) : undefined)
    })
...
export const uploadsApi = {
  async uploadVideos(files: File[]): Promise<ApiResponse<{ files: { name: string; url: string; size: number; type: string }[] }>> {
    const form = new FormData()
    files.forEach((f) => form.append('files', f))
    return apiRequest(`/api/uploads/video`, {
      method: 'POST',
      body: form,
    })
  }
